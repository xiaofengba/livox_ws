...
public:
    FastPointCloudRepublisher() : Node("fast_pointcloud_republisher")
    {
        ...
        // best_effort_qos.best_effort();  // 设置为尽力通信

        // 创建订阅的QoS配置 - 使用默认配置，但允许不匹配
        auto sub_qos = rclcpp::QoS(rclcpp::KeepLast(10));
        // sub_qos.best_effort();  // ReliabilityPolicy.BEST_EFFORT
        // sub_qos.durability_volatile();  // DurabilityPolicy.VOLATILE
        
        // 创建发布的QoS配置 - 尽力通信
        auto pub_qos = rclcpp::QoS(rclcpp::KeepLast(10));
        // pub_qos.best_effort();  // ReliabilityPolicy.BEST_EFFORT
        // pub_qos.durability_volatile();  // DurabilityPolicy.VOLATILE
        // pub_qos.best_effort();

        // 创建订阅和发布
        subscription_ = create_subscription<PointCloud2>(
            "/rslidar_points", sub_qos,
            [this](const PointCloud2::SharedPtr msg) { process_pointcloud(msg); });
        
        publisher_ = create_publisher<PointCloud2>("/rslidar_points_repub", pub_qos);
...
private:
    ...
    void convert_timestamp_memcpy(const uint8_t* input_point, uint8_t* output_point, double first_timestamp)
    {
        if (field_offsets_.count("timestamp")) {
            ...
            float relative_time = static_cast<float>((timestamp - first_timestamp));
